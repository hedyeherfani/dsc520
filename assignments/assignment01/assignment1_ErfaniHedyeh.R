...
week1_sleep <- c(6.1, 8.8, 7.4, 6.4, 6.2, 6.9, 6.6)
...
names(week1_sleep) <- days <- c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday")
names(week2_sleep) <- days <- c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday")
...
selection_vector <- week1_sleep_weekdays > 8
...
## Review
assignments<-colSums(grades)

## Total points for each student using `rowSums()`
students<-rowSums(grades)
