# Assignment: Github Assignment 2
# Name: Erfani, Hedyeh
# Date: 2020-06-07
...
dir(path_cwd)
setwd("/Users/hedyeherfani/IdeaProjects/untitled3")

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv('https://raw.githubusercontent.com/hedyeherfani/dsc520/master/data/tidynomicon/person.csv')
str(person_df1)
...
person_df2 <- read.csv('https://raw.githubusercontent.com/hedyeherfani/dsc520/master/data/tidynomicon/person.csv',stringsAsFactors = FALSE)
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv ('https://raw.githubusercontent.com/hedyeherfani/dsc520/master/data/scores.csv')
summary(scores_df)
...
voter_turnout_df2 <-read_excel('G04ResultsDetail2004-11-02.xls', sheet = "Voter Turnout",
                               col_names = c("ward_precinct","ballots_cast","registered_voters","voter_turnout") ,skip = 2)
